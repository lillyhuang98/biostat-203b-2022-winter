...
Explain the output. Explain the meaning of `"$1"`, `"$2"`, and `"$3"` in this shell script. Why do we need the first line of the shell script?
    ...
    **solution:**
    The output is using the editor to type the command in the text and save it as a file `middle.sh`. Then the second      command is to make the file executable by the owner by setting the permission and displays `pg42671` file with end     lines of 20 and number lines of 5
    `"$1"` means `filename` variables, `"$2"` means `endl_line` variables , and `"$3"`means `num_lines` variables.
    The first line of the shell script `#!/bin/sh` is needed because it tells the systems to operature the script by       using `bin/sh` as the interpreter.
