...
**Solution:** `zcat` is for uncompress the data,`zless` is for 
`zmore` is for
...
```
**solution:** The above output list all .gz files in the core.
...
```{bash eval=F}
   for datafile in /mnt/mimiciv/1.0/core/admissions.csv.gz
    do
      zcat $datafile | cat $subject_id | wc -l
    done
```
...
**Solution:** `wget -nc` download the file from the webpage.
...
**Solution** The first command create the file `test 1.txt` with adding in `hello, world` text in, the second command create the file `test 2.txt` with adding in `hello, world` ,but when run multiple times the second command, there will be multiple times adding of the text to the file depends on the times of running command.
