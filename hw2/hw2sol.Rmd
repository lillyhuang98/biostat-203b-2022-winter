...
```{r}
labevents_tble %>%
  pivot_longer("Bicarbonate" : "White Blood Cells", names_to = "variables", values_to = "values") %>%
  select(-subject_id) %>%
  summary(values) %>%
  knitr::kable()
...
mimic_icu_cohort=
  ...
  mutate(thirty_day_mort = hadm_death <= 2592000) %>%
  print(width = Inf)
```
...
demoplot=
  ...
  group_by(ethnicity) %>%
  filter(ethnicity == "WHITE" | ethnicity == "BLACK/AFRICAN AMERICAN" | ethnicity == "ASIAN") %>%
