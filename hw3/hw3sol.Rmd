...
```{r}
  semi_join (labevents_tble, icustays_tble, by = c("subject_id", "hadm_id")) %>%
  show_query() %>%
  print(width = Inf)
```
...
```{r}
labevents_tble %>%
  semi_join (labevents_tble, HW2_tble, by = c("subject_id", "hadm_id")) %>%
...
labevents_tble <- labevents_tble %>%
  select(itemid, valuenum, subject_id) %>%
  pivot_wider(names_from = itemid, names_prefix = "id", values_from = valuenum) %>%
...
```{r}
chartevents_tble %>%
  semi_join (chartevents_tble, HW2_tble, by = c("subject_id", "hadm_id")) %>%
  show_query() %>%
  print(width = Inf)
```
