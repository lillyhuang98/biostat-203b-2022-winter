...
model <- split(icudata,icudata$thirtyday_mort)

ICU <- lapply(model, function (d) {
    indexes <- sample(1:nrow(d), size=0.8*nrow(d))
})

trainingset <- data[ ICU[[1]], ] 
testingset <-  data[ -ICU[[1]], ]
```
...
```{r}
Compare_models <-table(traininngset = test$thirtyday_mort, testingset = log2)
Compare_models
```
```{r}
comparelasso1 <- 
  predict(icuglm, 
          newx = model.matrix(
            test$thirtyday_mort ~ ., test),
          s = iculse, type = "response")
comparelasso2 <- ifelse(comparelasso1 > 0.8, TRUE, FALSE)
compare_models2 <- table(trainingset = test$thirtyday_mort, testingset = comparelasso2)
compare_models2
```
